...
// Função para exibir avisos baseados no protocolo atual
function showProtocolWarnings() {
  // Avisos sobre protocolo file:// e GitHub Pages
  const isFileProtocol = window.location.protocol === 'file:';
  
  // Aviso de acesso local (file://)
  const localWarning = document.getElementById('local-warning');
  if (localWarning) {
    localWarning.style.display = isFileProtocol && !appState.isGitHubPages ? 'block' : 'none';
  }
  
  // Aviso do GitHub Pages
  const githubWarning = document.getElementById('github-warning');
  if (githubWarning) {
    githubWarning.style.display = appState.isGitHubPages ? 'block' : 'none';
  }
}
...
function loadSectionContent(sectionId) {
  ...
  // Tenta carregar o conteúdo usando diferentes abordagens, dependendo do protocolo
  if (appState.isFileProtocol) {
    // Array de tentativas de caminhos para o arquivo
    const pathsToTry = [
      filePath,                          // Caminho original
      `../${filePath}`,                  // Um nível acima
      `../../${filePath}`,               // Dois níveis acima
      `./${filePath}`,                   // Caminho relativo explícito
      `/${filePath}`,                    // Caminho absoluto a partir da raiz
      window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/') + 1) + filePath // Caminho baseado na URL atual
    ];
    ...
    function tryNextPath() {
      if (pathIndex >= pathsToTry.length) {
        // Esgotamos todas as tentativas, usar fallback
        if (appState.debug) console.log(`Todas as tentativas de carregamento falharam para ${sectionId}, usando fallback`);
        loadStaticFallbackContent(sectionId, contentPlaceholder);
        return;
      }
      
      const currentPath = pathsToTry[pathIndex];
      if (appState.debug) console.log(`Tentativa ${pathIndex + 1}/${pathsToTry.length}: Carregando de ${currentPath}`);
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
          if ((xhr.status === 200 || (xhr.status === 0 && xhr.responseText)) && xhr.responseText.length > 100) {
            // Sucesso! Temos conteúdo válido
            if (appState.debug) console.log(`Conteúdo carregado com sucesso de ${currentPath}`);
            insertContent(xhr.responseText, contentPlaceholder, sectionId);
            appState.loadedSections[sectionId] = true;
            
            // Ocultar aviso se carregamento bem-sucedido
            if (fileProtocolNotice) {
              fileProtocolNotice.style.display = 'none';
            }
          } else {
            // Falha, tentar próximo caminho
            pathIndex++;
            tryNextPath();
          }
        }
      ...
      xhr.onerror = function() {
        // Erro na requisição, tentar próximo caminho
        if (appState.debug) console.log(`Erro ao carregar de ${currentPath}`);
        pathIndex++;
        tryNextPath();
      };
    ...
    // Para protocolo HTTP normal, usar fetch
    fetch(filePath)
      .then(response => {
        if (!response.ok) {
...
// Carregar conteúdo estático de fallback para protocolo file://
function loadStaticFallbackContent(sectionId, contentPlaceholder, returnHTML = false) {
  if (appState.debug && !returnHTML) console.log(`Usando conteúdo estático mínimo para ${sectionId} como último recurso`);
  
  // Conteúdo estático mínimo para cada seção - apenas como último recurso
  const staticContents = {
    'sumario-executivo': `
      <div class="error-message">
        <h3>Conteúdo não carregado</h3>
        <p>Não foi possível carregar o conteúdo completo do sumário executivo.</p>
    ...
    'analise-mercado': `
      <div class="error-message">
        <h3>Conteúdo não carregado</h3>
        <p>Não foi possível carregar o conteúdo completo da análise de mercado.</p>
        <p>Para visualizar o conteúdo completo, utilize um servidor HTTP local:</p>
        <pre>python -m http.server 8000</pre>
        <p>E acesse <a href="http://localhost:8000">http://localhost:8000</a> no seu navegador.</p>
      </div>
  ...
  if (returnHTML) {
    return staticContents[sectionId] || `
      <div class="error-message">
        ...
        <p>Não foi possível carregar o conteúdo completo para ${sectionId.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}.</p>
        <p>Para visualizar o conteúdo completo, utilize um servidor HTTP local.</p>
      </div>
    `;
  }
  
  // Inserir conteúdo estático ou mensagem de erro
  contentPlaceholder.innerHTML = staticContents[sectionId] || `
    <div class="error-message">
      <h3>Conteúdo não disponível</h3>
      <p>Não foi possível carregar o conteúdo desta seção quando o relatório é acessado diretamente pelo sistema de arquivos.</p>
      <p>Para visualizar o conteúdo completo, utilize um servidor HTTP local:</p>
      <pre>python -m http.server 8000</pre>
...
function insertContent(html, placeholder, sectionId) {
  ...
  // Disparar evento customizado para inicializar gráficos
  document.dispatchEvent(new CustomEvent('amica:initCharts', {
